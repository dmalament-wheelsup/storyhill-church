...
- **The sheet has NO header row** and column meaning is positional: A=name (the card **title**), B=time (free text, e.g. "Tuesday mornings" / "Wed 9:30 AM"), C=coordinates, D=location (the card **subtitle**), E=description, F=comma-separated tags, G=comma/semicolon-separated contact email(s). `loadGroups()` maps these by index — changing column order means editing the index mapping in `loadGroups()`. Rows are dropped unless they have a `name` (column A). (If a header row is ever added to the sheet, flip the gviz URL from `headers=0` to `headers=1` so the page skips it — see `SHEET-SETUP.md`.)
- **Mail — Cloudflare Worker + Resend** (`worker/`). The contact form POSTs JSON to `CONFIG.CONTACT_ENDPOINT` (the Worker); the Worker validates and sends via Resend. The page sends the group's raw column-G recipients (possibly empty); the **Worker** enforces always-CCing `grow@storyhill.org` (its `ALWAYS_TO` var) and uses it alone when the list is empty — that rule lives server-side, not in the page. The Worker also does origin allowlisting and honeypot (`_honey`) filtering. Why a backend at all: per-group recipients made FormSubmit unworkable (it requires per-address confirmation), so the key/send moved server-side. `worker/README.md` has the deploy steps.
...
- **Iframe embedding** (`initAutoResize`/`postHeight` + the `storyhill:*` `postMessage` protocol): the page reports its **body** height to the parent so the iframe auto-sizes — measure `document.body`, never `documentElement`, whose `scrollHeight` reflects the externally-applied iframe height and would ratchet upward and never shrink. The parent reports its visible band back so modals center on screen, and modal open/close posts a scroll-lock. All of it no-ops when `window.parent === window`. The matching parent-side snippet lives in the README; keep the two in sync if you change message shapes.
